...
extends the objective critical path through completed V1/V2 engineering and
terminates the current project at an explicit existing-evidence closure gate.
Untouched future V2 validation remains optional future research infrastructure,
not a blocker for this project's completion.
"""
...
def derive_closed_state(issues: list[dict]) -> dict:
    """Derive state while preserving an explicit terminal closure state."""
    ...
    if closed:
        # Base state historically falls back to SPEC_READY when all gates pass.
        # A closed project must remain visibly closed instead of regressing.
