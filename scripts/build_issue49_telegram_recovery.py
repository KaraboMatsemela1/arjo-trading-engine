#!/usr/bin/env python3
"""Recover bounded Issue #49 cue windows from only Telegram posts 776 and 791.

The archive is used only as a replayable transport for two canonical captured
first-party sources. No other Telegram message can receive semantic credit.
"""
...
TARGET_SOURCE_IDS = {"TG_ARJOIOTRADING_776", "TG_ARJOIOTRADING_791"}
...
CONCEPT_ALIASES = {
    "PD_ARRAYS": ["PD Array", "PD Arrays"],
    "TWO_CANDLE_REJECTION": ["2 Candle Rejection", "2CR"],
    "NO_RESISTANCE_AREA": ["No Resistance Area", "No Resistance"],
    "CANDLE_SCIENCE": ["Candle Science"],
}

FIELD_CUES = {
    "INSTRUMENTS": re.compile(r"\b(?:gold|xau(?:usd)?|eurusd|gbpusd|usdjpy|nq|es|dxy|btc|eth)\b", re.IGNORECASE),
    "TIMEFRAME": re.compile(r"\b(?:monthly|weekly|daily|4h|1h|2h|3h|15m|30m|5m|1m|htf|ltf|higher timeframe|lower timeframe)\b", re.IGNORECASE),
    "HIGHER_TIMEFRAME_CONTEXT": re.compile(r"\b(?:htf|higher timeframe|daily|weekly|monthly|4h|1h)\b", re.IGNORECASE),
    "DIRECTION": re.compile(r"\b(?:direction|buy|buys|sell|sells|long|longs|short|shorts|bullish|bearish|higher|lower)\b", re.IGNORECASE),
    "PRECONDITIONS_SETUP": re.compile(r"\b(?:opposing|resistance|no resistance|respect|disrespect|hold|holds|holding|fail|fails|failed|failure|pd array|pd arrays|rejection)\b", re.IGNORECASE),
    "TRIGGER_ENTRY": re.compile(r"\b(?:trigger|entry|entries|enter|confirmation|involved|involvement|2cr|2 candle rejection|rejection)\b", re.IGNORECASE),
    ...
    "INVALIDATION_EXPIRY": re.compile(r"\b(?:invalid|invalidate|invalidation|fail|fails|failed|failure|expire|expires|expiry|until)\b", re.IGNORECASE),
    ...
    "OPTIONAL_REQUIRED": re.compile(r"\b(?:need|required|must|optional|confirmation|combine|using|with)\b", re.IGNORECASE),
}
...
def eligible_targets(path: Path) -> set[str]:
...
def persist_archive_page(cache_root: Path | None, page: str, page_index: int, current_before: int | None, url: str) -> dict[str, str]:
...
def main() -> int:
    ...
    parser.add_argument("--max-excerpts-per-message", type=int, default=16)
    args = parser.parse_args()

    eligible = eligible_targets(Path(args.acquisition))
    if eligible != TARGET_SOURCE_IDS:
        missing = sorted(TARGET_SOURCE_IDS - eligible)
        raise SystemExit(f"Issue 49 bounded Telegram targets are not canonical captured first-party payloads: {missing}")
    ...
    for page_index in range(args.max_pages):
        if current_before in visited_before or found_targets == TARGET_SOURCE_IDS:
            break
        ...
        for match in MESSAGE_RE.finditer(page):
            ...
            if source_id not in TARGET_SOURCE_IDS:
                continue
            found_targets.add(source_id)
            body = match.group("body")
            text = clean_text(body)
            concepts = concept_hits(text)
            cue_hits = {field: list(pattern.finditer(text)) for field, pattern in FIELD_CUES.items()}
    ...
    missing_targets = sorted(TARGET_SOURCE_IDS - found_targets)
    report = {
        "schema_version": 1,
        "issue": 49,
        ...
        "missing_bounded_targets": missing_targets,
        "failures": failures,
    ...
    print(json.dumps({"eligible": len(eligible), "pages": pages_fetched, "messages": len(rows), "missing_targets": missing_targets, "failures": len(failures)}, sort_keys=True))
    return 4 if failures or missing_targets else 0
