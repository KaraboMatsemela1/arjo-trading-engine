...
def pf_value(m: dict) -> float:
    return (
        math.inf
        if m["no_negative_trades"] and m["positive_r_sum"] > 0
        else float(m["profit_factor"] or 0.0)
    )


def classify(base: dict, stress: dict) -> str:
    if base["resolved_executed_trades"] < 350:
    ...
    passed = (
        ...
        and pf_value(base) > 1.40
        and lower is not None
        and lower > 0
        and stress["net_expectancy_r"] is not None
        and stress["net_expectancy_r"] > 0
        and pf_value(stress) > 1.15
        and (base["positive_calendar_year_fraction"] or 0) >= 0.80
    ...
    return (
        "V8_FVG_FOLLOWTHROUGH_HISTORICAL_EDGE_ESTABLISHED"
        if passed
        else "EDGE_NOT_ESTABLISHED"
    )
