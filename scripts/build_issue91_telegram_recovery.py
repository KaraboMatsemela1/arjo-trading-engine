#!/usr/bin/env python3
"""Recover bounded Issue #91 cue windows from canonical Telegram post 16."""
...
TARGET_SOURCE_ID = "TG_ARJOIOTRADING_16"
...
CUES = {
    ...
    "STOP_RUN": re.compile(r"\bstop\s+run\b", re.IGNORECASE),
    "INSTRUMENTS": re.compile(r"\b(?:gold|xau(?:usd)?|eurusd|gbpusd|nq|es|dxy|btc|eth|bonds?|dollar|indices|index|forex)\b", re.IGNORECASE),
    ...
    "DIRECTION": re.compile(r"\b(?:bullish|bearish|buy|sell|long|short|higher|lower|upside|downside)\b", re.IGNORECASE),
    "TRIGGER": re.compile(r"\b(?:trigger|confirmation|confirm|break|sweep|run|respect|disrespect|hold|fail)\b", re.IGNORECASE),
    "ENTRY": re.compile(r"\b(?:entry|enter|involv(?:e|ed|ement))\b", re.IGNORECASE),
    "TARGET": re.compile(r"\b(?:target|draw on liquidity|liquidity)\b", re.IGNORECASE),
    "INVALIDATION": re.compile(r"\b(?:invalidate|invalidation|invalid|fail|failure)\b", re.IGNORECASE),
...
def target_is_eligible(path: Path) -> bool:
    for record in read_jsonl(path):
        ...
        return bool(
            ...
            and record.get("sha256") == "65ea7c0cec45e16fe568bd1f3dd83867b3fd11051c3f2914c336dbb2634d0a79"
        )
    return False
...
def recover_message(body: str, page_meta: dict[str, str], max_words: int, max_excerpts: int) -> dict:
    ...
    for label, pattern in CUES.items():
        for hit in list(pattern.finditer(text))[:3]:
            ...
            if not excerpt or key in seen or contains_pre_spec_outcome(excerpt):
                continue
            seen.add(key)
...
def main() -> int:
    ...
    if not target_is_eligible(Path(args.acquisition)):
        raise SystemExit("Issue 91 Telegram target is not the canonical captured first-party payload")
    ...
    print(json.dumps({
        ...
        "missing": 0 if recovered else 1,
        "failures": len(failures),
        "matched_cues": recovered["matched_cues"] if recovered else [],
        "excerpts": recovered["excerpts"] if recovered else [],
